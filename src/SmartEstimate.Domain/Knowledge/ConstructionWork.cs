...
/// <summary>Aggregate root representing a selectable construction work.</summary>
public sealed class ConstructionWork : KnowledgeRecord
{
    private ConstructionWork()
    {
    }

    private ConstructionWork(Guid id, LocalizedText name, string? description, Guid categoryId, Guid unitId, IEnumerable<string>? tags, KnowledgeStatus status, DateTimeOffset createdAt, Guid? actorId)
        : base(id, status, createdAt, actorId)
    {
        Name = name;
        Description = NormalizeDescription(description);
        CategoryId = RequiredId(categoryId, nameof(categoryId));
        UnitId = RequiredId(unitId, nameof(unitId));
    ...
    public static ConstructionWork Create(Guid id, LocalizedText name, string? description, Guid categoryId, Guid unitId, IEnumerable<string>? tags, KnowledgeStatus status, DateTimeOffset createdAt, Guid? actorId) =>
    ...
    private static Guid RequiredId(Guid value, string parameterName) => value == Guid.Empty ? throw new ArgumentException("A reference is required.", parameterName) : value;
